...
## Progreso verificado
- Refactor: `.tmp/verification/workflow-refactor-2026-09-21/verification.json`. Casos de control: orden normal, decisiones/baseline bloqueantes, aprobación del plan, reintento por checks, Ralph por subtareas, feedback de review/QA, agotamiento y rechazo de candidato modificado. La lectura del run histórico `2026-09-20T18-21-18-110Z-escape-search-acceptance-url-normal` sigue funcionando con `workflow status`. Rechazar una reanudación conserva el estado anterior; el lock se libera también ante errores.
