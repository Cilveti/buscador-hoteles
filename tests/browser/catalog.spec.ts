...
test('buscar, abrir detalle y volver conserva la consulta', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Hotel Demo Málaga' })).toBeVisible();
...
test('país, orden, paginación y recarga consultan el handler real', async ({ page }) => {
  ...
  await expect(
    page.getByRole('link', { name: 'Ver hotel Hotel Demo Sevilla', exact: true }),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Página siguiente', exact: true }).click();
  await expect(
    page.getByRole('link', { name: 'Ver hotel Hotel Demo Málaga', exact: true }),
  ).toBeVisible();
  await page.reload();
  await expect(
    page.getByRole('link', { name: 'Ver hotel Hotel Demo Málaga', exact: true }),
