...
5. Verificar mediante una llamada de lectura el nombre/ID del archivo y que la página esté vacía. Leer `high_level_overview` y la documentación de API que corresponda. Ejecutar el contenido de `factory/penpot/create-demo.js` usando `execute_code` en ese archivo.
6. Exportar los tableros con `export_shape` en SVG y PNG. Extraer `penpot.library.local.tokens.sets` y la estructura/estilos del archivo real. Guardar una versión con nombre y registrar su etiqueta, URL y fecha en un nuevo manifiesto. Revisar imágenes y hashes.
